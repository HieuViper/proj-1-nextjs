...
export async function PUT(req, { params }) {
    //Check for security
    const { reqStatus, loginInfo } = await funcLogin.checkForProtectedApi('news_imgs', 'edit');
    if ( reqStatus != 200 )
    ...
    try {
        ...
        await newsImgs.updateImgById( imgData, imageInfo.id );
        ...
        result.images = await newsImgs.getAllImages( query.page, query.size, query.search );
        result.totals = await newsImgs.getTotalNumOfImg( query.search );
        result.pagination = {
