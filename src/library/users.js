...
async function userList( loginInfo, searchParams ) {
  ...
  try {
      if( keys != '' || del != '' ) {
        ...
        if ( keys != '' )
            await users.deleteBulkUsers(keys, loginInfo);
        if ( del != '' )
            await users.deleteUser(del, loginInfo);
      }
      result.users = await users.getUsers(
          role,
          page,
          size,
          search,
      ...
      result.totals = await users.getTotalNumOfUsers( role, search );
      result.pagination = {
